...
最常见的复用异常类型是 IllegalArgumentException（[Item-49](./Chapter-8-Item-49-Check-parameters-for-validity)）。这通常是调用者传入不合适的参数时抛出的异常。例如，如果调用者在表示某个操作要重复多少次的参数中传递了一个负数，则抛出这个异常。
...
- **Previous Item（上一条目）：[Item 71: Avoid unnecessary use of checked exceptions（避免不必要地使用 checked 异常）](./Chapter-10/Chapter-10-Item-71-Avoid-unnecessary-use-of-checked-exceptions)**
- **Next Item（下一条目）：[Item 73: Throw exceptions appropriate to the abstraction（抛出能用抽象解释的异常）](./Chapter-10/Chapter-10-Item-73-Throw-exceptions-appropriate-to-the-abstraction)**
