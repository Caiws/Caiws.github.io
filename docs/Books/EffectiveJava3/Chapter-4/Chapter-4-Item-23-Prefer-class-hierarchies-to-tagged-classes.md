...
注意，上面层次结构中的字段是直接访问的，而不是通过访问器方法访问的。这样做是为了简洁，如果层次结构是公共的，那么这将是一个糟糕的设计（[Item-16](./Chapter-4-Item-16-In-public-classes-use-accessor-methods-not-public-fields)）。
...
- **Previous Item（上一条目）：[Item 22: Use interfaces only to define types（接口只用于定义类型）](./Chapter-4-Item-22-Use-interfaces-only-to-define-types)**
- **Next Item（下一条目）：[Item 24: Favor static member classes over nonstatic（静态成员类优于非静态成员类）](./Chapter-4-Item-24-Favor-static-member-classes-over-nonstatic)**
