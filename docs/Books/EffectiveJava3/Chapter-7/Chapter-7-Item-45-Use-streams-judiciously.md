...
默认情况下，流管道按顺序运行。让管道并行执行与在管道中的任何流上调用并行方法一样简单，但是这样做不一定合适（[Item-48](./Chapter-7-Item-48-Use-caution-when-making-streams-parallel)）。
...
- **Previous Item（上一条目）：[Item 44: Favor the use of standard functional interfaces（优先使用标准函数式接口）](./Chapter-7-Item-44-Favor-the-use-of-standard-functional-interfaces)**
- **Next Item（下一条目）：[Item 46: Prefer side effect free functions in streams（在流中使用无副作用的函数）](./Chapter-7-Item-46-Prefer-side-effect-free-functions-in-streams)**
