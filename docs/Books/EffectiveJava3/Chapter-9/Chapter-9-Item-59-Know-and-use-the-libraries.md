...
- **Previous Item（上一条目）：[Item 58: Prefer for-each loops to traditional for loops（for-each 循环优于传统的 for 循环）](./Chapter-9-Item-58-Prefer-for-each-loops-to-traditional-for-loops)**
- **Next Item（下一条目）：[Item 60: Avoid float and double if exact answers are required（若需要精确答案就应避免使用 float 和 double 类型）](./Chapter-9-Item-60-Avoid-float-and-double-if-exact-answers-are-required)**
