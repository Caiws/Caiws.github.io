...
**在大多数情况下，常规初始化优于延迟初始化。** 下面是一个使用常规初始化的实例字段的典型声明。注意 final 修饰符的使用（[Item-17](./Chapter-4-Item-17-Minimize-mutability)）：
...
- **Previous Item（上一条目）：[Item 82: Document thread safety（文档应包含线程安全属性）](./Chapter-11/Chapter-11-Item-82-Document-thread-safety)**
- **Next Item（下一条目）：[Item 84: Don’t depend on the thread scheduler（不要依赖线程调度器）](./Chapter-11/Chapter-11-Item-84-Don’t-depend-on-the-thread-scheduler)**
